...
public:
    ProtoEditorApp()
    {
        ...
        main_scene->setSkybox(
            Engine::loadTexture("res://engine/samples/nasa_goddard_gaia_dr2_deep_star_map.png"));
    ...
    {
        node_view->checkInput(
            { 0, node_scene->getViewportSize().y * 3.0f }, node_scene->getViewportSize());
        if (Input::getMousePosition().y < node_scene->getViewportSize().y * 3.0f)
            asha_app->update(delta_time);
    }

    void drawImGui() override { asha_app->drawImGui(); }
};
