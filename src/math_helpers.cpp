...
float HopEngine::intersect(glm::vec3 ray_origin, glm::vec3 ray_direction, const BoundingBox& bounding_box, Transform& transform)
{
    ...
    // based on this: https://www.opengl-tutorial.org/miscellaneous/clicking-on-objects/picking-with-custom-ray-obb-function/
    ...
    t1 = t1s.y;
    t2 = t2s.y;
    if (t1 > t2) { t1 = t2s.y; t2 = t1s.y; }
    t_max = std::min(t2, t_max);
    t_min = std::max(t1, t_min);
    
    t1 = t1s.z;
    t2 = t2s.z;
    if (t1 > t2) { t1 = t2s.z; t2 = t1s.z; }
    t_max = std::min(t2, t_max);
    t_min = std::max(t1, t_min);
