...
DrawCommandBuffer::DrawCommandBuffer()
{
    ...
    if (vkAllocateCommandBuffers(RenderServer::getDevice(), &buffer_allocate_info, &buffer) != VK_SUCCESS)
    ...
    if (vkCreateQueryPool(RenderServer::getDevice(), &query_pool_create_info, nullptr, &query_pool) != VK_SUCCESS)
...
void DrawCommandBuffer::begin(uint32_t index, FrameStats* frame_stats)
{
    ...
    if (vkBeginCommandBuffer(buffer, &cmd_buffer_begin_info) != VK_SUCCESS)
        DBG_FAULT("vkBeginCommandBuffer failed");
    
    query_offset = 0;
    vkCmdResetQueryPool(buffer, query_pool, 0, 512);
...
void DrawCommandBuffer::bindPipelineInternal(GPUHandle pipeline)
{
    ...
    vkCmdBindPipeline(buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, static_cast<VkPipeline>(pipeline));
...
void DrawCommandBuffer::bindIndexBuffer(GPUHandle index_buffer)
{
    ...
    vkCmdBindIndexBuffer(buffer, static_cast<VkBuffer>(index_buffer), 0, VK_INDEX_TYPE_UINT16);
...
void DrawCommandBuffer::extractTiming()
{
    ...
    vkGetQueryPoolResults(RenderServer::getDevice(), query_pool, 0, query_offset, results_buf.size() * sizeof(uint32_t), results_buf.data(), 4, VK_QUERY_RESULT_WAIT_BIT);
...
{
    vkCmdWriteTimestamp(buffer, bottom_of_pipe ? VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT : VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, query_pool, query_offset++);
}
