...
DataBlock Package::load(const std::string& path)
{
    ...
    if (isResPath(path, real_path))
    {
        ...
        if (redirector == instance->alias_table.end()) it = instance->database.find(real_path);
        else
            it = instance->database.find(redirector->second);

        if (it != instance->database.end()) return it->second;

        if (redirector == instance->alias_table.end())
            DBG_WARNING("found no data associated with '" + real_path + "'");
        else
            DBG_WARNING("found no data associated with '" + real_path + "' (redirected to '" +
                        redirector->second + "')");
...
DataBlock Package::loadFromDisk(const std::string& path)
{
...
bool Package::storeToDisk(const std::string& path, const DataBlock& data)
{
    DBG_VERBOSE("storing '" + path + "' to file; " + std::to_string(data.size()) + " bytes");
...
bool Package::importPackage(const DataBlock& data)
{
    ...
    if (header.signature_version == SIGNATURE + 2)
    {
        ...
        std::filesystem::create_directories(Package::getTempPath());
        std::string temp_zip_address =
            Package::getTempPath() + "hop_package_tmp" + PTR(instance) + ".zip";
...
std::vector<std::string> Package::listLoadedEntries()
{
    std::vector<std::string> names;
    names.reserve(instance->database.size());
    for (const auto& [identifier, _] : instance->database) names.push_back(identifier);
    return names;
}

void Package::setAlias(const std::string& a, const std::string& b) { instance->alias_table[a] = b; }

void Package::clearAlias(const std::string& a) { instance->alias_table.erase(a); }
