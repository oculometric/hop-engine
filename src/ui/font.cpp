...
Font::Font(const std::string& atlas_name, const glm::ivec2 glyph_size_pixels, const std::string& bold_atlas_name)
{
    std::vector<Ref<Texture>> atlases;
    atlases.push_back(Texture::loadImage(atlas_name));
    if (!bold_atlas_name.empty())
        atlases.push_back(Texture::loadImage(bold_atlas_name));
