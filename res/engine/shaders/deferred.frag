...
void main()
{
    ...
    vec4 albedo_val = texture(albedo_tex, frag.uv);
    ...
    vec3 normal_val = normalize((toSRGB(texture(normal_tex, frag.uv).rgb) * 2.0f - 1.0f));
    normal_val.y *= -1;
    vec3 perturbed_normal = tbn * normal_val;
    
    vec4 pbr_val = texture(pbr_tex, frag.uv);
    ...
    // out_colour.a   <- emission strength
    ...
    // out_custom.a   <- EMPTY
    out_colour = vec4(albedo_val.rgb * frag.colour.rgb, pbr_val.z);
    out_normal = vec4(normalize(mix(frag.normal, perturbed_normal, normal_strength)), 0.0f);
    out_params = vec4(pbr_val.xy, 0.0f, 0.0f);
